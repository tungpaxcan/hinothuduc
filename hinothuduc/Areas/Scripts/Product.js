...
function Product(pagenum, page, seach) {
    var idcate = $('#idcate').val().trim();
    var Stt = 1;
    $.ajax({
        url: '/product/List',
        type: 'get',
        data: { pagenum, page, seach, idcate },
        success: function (data) {
            ...
            if (data.code == 200) {
                $.each(data.c, function (k, v) {
                    ...
                    table += '<a class="nav-link" href="/Hino/Product/Image/' + v.id + '">';
                    table += '<img style="width:16px" src="/Images/icons8-image-16.png" />';
                    table += '<span class="nav-text"> Thêm Ảnh</span></a></li>';
                    table += ' <li class="nav-item">';
                    table += '<a class="nav-link" href="/Hino/Product/Edits/' + v.id + '">';
...
function Add() {
    ...
    $.ajax({
        ...
        type: 'post',
        data: {
            idcate, name, title, meta, image, content
        },
        success: function (data) {
            if (data.code == 200) {
                Swal.fire({
                ...
                window.location.href = "/Hino/Product/Index/" + idcate;
